...
const ChatEndStudyActions = () => {
    const onBackToSelectClick = () => store.dispatch(ChatStudyDeckActions.reset());

    const onEndRoomClick = () => store.dispatch(ChatActions.setModal({ type: MODALS.END_ROOM }));

    return (
        <div className={styles.chatEndStudyActions}>
            <div className={styles.actionsText}>Solid study sesh. Now what?</div>
            <div className={styles.buttons}>
                <button className={`${styles.btn} ${styles.endRoomBtn}`} onClick={onEndRoomClick}>
                    End Room
                </button>
                <button
