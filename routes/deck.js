...
router.post('/api/decks/create', bodyParser.json(), async (req, res) => {
    ...
    try {
        ...
        console.log(
            '\n \n result inside create deck controller ',
            result,
            '\n'
        );
...
router.get('/api/decks/user/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const response = await Deck.find({ userId: id }).sort({
            timestamp: -1,
        });
        // console.log(
        //     '\n \n response from getting all user decks ',
        //     response,
        //     '\n'
        // );
        res.json({ decks: response, isSuccess: true });
...
router.put('/api/decks/edit-favorites', async (req, res) => {
    ...
    try {
        ...
        console.log(
            '\n \n response from editing favs for a deck ',
            response,
            '\n'
        );
        ...
        console.log(
            '\n error in editing number of favs for a deck = ',
            e,
            '\n\n'
        );
        res.status(400).send({ isSuccess: false, error: e });
